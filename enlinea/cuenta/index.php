...
switch ($accion) {
    ...
    // <editor-fold defaultstate="collapsed" desc="consulta solvencia de condomino">
    case "solvencia":
        ...
        if ($propiedades['suceed'] && count($propiedades['data']) > 0) {
            for ($index = 0; $index < count($propiedades['data']); $index++) {
                $inmueble = $inmuebles->ver($propiedades['data'][$index]['id_inmueble']);
                if ($inmueble['suceed'] && count($inmueble['data']) > 0) {
                    $propiedades['data'][$index]['inmueble'] = $inmueble['data'];
                }
                //$resultado = $facturas->ultimoPeriodoFacturado($propiedades['data'][$index]['id_inmueble']);
                //$propiedades['data'][$index]['periodo'] = $resultado;
            }
        }
        echo $twig->render('enlinea/cuenta/solvencia.html.twig', array(
            "session" => $session,
